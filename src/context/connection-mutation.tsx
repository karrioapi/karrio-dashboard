...
import { CreateCarrierConnectionMutationInput, create_connection, CREATE_CARRIER_CONNECTION, create_connectionVariables, create_connection_create_carrier_connection, delete_connection, DELETE_CARRIER_CONNECTION, delete_connectionVariables, delete_connection_delete_carrier_connection, UpdateCarrierConnectionMutationInput, update_connection, UPDATE_CARRIER_CONNECTION, update_connectionVariables, update_connection_update_carrier_connection } from 'karrio/graphql';
...
const ConnectionMutationProvider: React.FC<{}> = ({ children }) => {
  const [createMutation] = useMutation<create_connection, create_connectionVariables>(CREATE_CARRIER_CONNECTION);
  const [updateMutation] = useMutation<update_connection, update_connectionVariables>(UPDATE_CARRIER_CONNECTION);
  const [deleteMutation] = useMutation<delete_connection, delete_connectionVariables>(DELETE_CARRIER_CONNECTION);
